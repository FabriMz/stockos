<template>
  <div class="screen" v-if="brand || isSinMarca">
    <TopBar variant="back" :back-label="config.title" :back-to="config.basePath"
      :title="isSinMarca ? 'Sin marca' : brand.name" />

    <AlertSearchBar v-model="searchQuery" input-id="alerts-brand-products-search" placeholder="Buscar producto…" />

    <div class="scroll-content">
      <!-- Cabecera de marca -->
      <div class="brand-summary">
        <div class="brand-summary__icon" :style="isSinMarca ? {} : { background: brand.bg }">
          <i :class="`ti ${isSinMarca ? 'ti-box' : brand.ic}`" :style="isSinMarca ? {} : { color: brand.col }"
            aria-hidden="true"></i>
        </div>
        <div>
      ...
      <template v-if="categorizedAlertProducts.length">
        <div v-for="group in categorizedAlertProducts" :key="group.cat ?? '__sin_cat__'" class="cat-accordion"
          :class="{ 'cat-accordion--open': catOpen.isOpen(route.params.brandId, group.cat ?? '__sin_cat__') }">
          <!-- Header del acordeón -->
          <div class="cat-accordion__header" role="button" tabindex="0"
            :aria-expanded="String(catOpen.isOpen(route.params.brandId, group.cat ?? '__sin_cat__'))"
            :aria-controls="`alerts-cat-body-${group.cat ?? '__sin_cat__'}`" :aria-label="catOpen.isOpen(route.params.brandId, group.cat ?? '__sin_cat__')
          ...
          <!-- Cuerpo colapsable -->
          <div :id="`alerts-cat-body-${group.cat ?? '__sin_cat__'}`" class="cat-accordion__body" role="region"
            :aria-label="group.cat ?? 'Sin categoría'">
            <div v-for="p in group.items" :key="p.id" class="brand-row" role="button" tabindex="0" :aria-label="p.name"
              ...
              <div class="brand-row__body">
                <div class="brand-row__header">
                  <div class="brand-row__icon" :style="isSinMarca ? {} : { background: brand.bg }">
                    ...
                    <i v-else :class="`ti ${isSinMarca ? 'ti-box' : brand.ic}`"
                      :style="isSinMarca ? {} : { color: brand.col }" aria-hidden="true"></i>
                  </div>
                  <div class="brand-row__info brand-row__info--wrap">
                    <div class="brand-row__name">{{ p.name }}</div>
                    <div class="brand-row__meta">
                      SKU {{ p.sku }} · Lote {{ p.batch || 'S/N' }}<template v-if="config.showStock"> · {{ p.stock }}
                        uds.</template>
                    </div>
                  </div>
                </div>
